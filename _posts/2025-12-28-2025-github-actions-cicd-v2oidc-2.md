---
title: "GitHub Actions로 “안전하고 빠른” CI/CD 파이프라인 구축하는 법 (캐시 v2·OIDC·권한 최소화까지)"
description: "2025년의 CI/CD는 “돌아가기만 하면 된다”를 넘어, 속도(캐시/병렬화)·안정성(동시성 제어)·보안(권한 최소화/비밀 제거)가 기본 요구사항이 됐습니다."
...
즉, “배포 job”만 cloud 권한이 필요하고, “CI job”은 코드 읽기 정도만 필요합니다. GitHub Docs는 `permissions`로 `GITHUB_TOKEN` 스코프를 세밀히 제한할 수 있고, 지정하지 않은 권한은 `none`이 된다고 명시합니다.[^1]
...
즉, **GitHub Secrets에 장기 키를 저장하지 않아도 배포 가능**해지고, 토큰이 자동 만료되어 사고 반경이 줄어듭니다.[^2]
...
공식 문서에 따르면 그룹당 **동시에 1 running + 1 pending**만 유지되며, 새 pending이 생기면 기존 pending은 취소되는 동작도 설명돼 있습니다.[^3]

### 4) 캐시(actions/cache): 2025년에 더 중요해진 “러너/액션 버전 호환성”
`actions/cache`는 2025년을 기점으로 **캐시 백엔드 서비스가 v2 API로 전환**되고, 특정 버전 미업그레이드 시 워크플로가 실패할 수 있음을 명시하고 있습니다. 또한 self-hosted runner는 최소 runner 버전 요구사항이 붙습니다.[^4]
...
- `permissions`를 job별로 쪼개서 **권한 경계**를 만든다.[^1]  
- `id-token: write`는 deploy job에만 주고, OIDC로 **장기 secret 없는 배포**로 전환한다.[^2]

---

## ⚡ 실전 팁
1) **캐시 액션/러너 버전은 “운영 요소”로 관리**
- `actions/cache`는 2025년 캐시 서비스 전환 이슈(백엔드 재작성, 롤아웃/선셋 등)와 runner 최소 버전 요구사항이 명시되어 있습니다. self-hosted를 쓴다면 runner 업그레이드가 늦는 순간 캐시 단계가 파이프라인의 단일 장애점이 됩니다.[^4]  
...
- prod 배포는 반대로 “중간 취소가 더 위험”할 수 있으니 `cancel-in-progress: false`로 직렬화만 하고, 배포 안정성을 택하는 경우가 많습니다. (공식 동작 방식은 문서에 상세히 설명되어 있습니다.)[^3]

3) **`GITHUB_TOKEN`은 기본적으로 ‘과하게 강력’하다고 가정**
- 문서에서도 액션이 `github.token` 컨텍스트로 토큰에 접근할 수 있음을 강조합니다. 즉, “내가 명시적으로 넘기지 않았으니 안전”이 아닙니다.[^5]  
- 대응: workflow/job에 `permissions`를 명시해서 최소 권한을 강제하세요.

4) **조직 차원의 “SHA pinning required”도 고려**
- GitHub REST API에는 repo 수준에서 `sha_pinning_required` 같은 정책을 설정하는 항목이 있습니다. 공급망 공격 방어 관점에서 “조직 정책으로 강제”할 여지가 있다는 뜻입니다.[^6]  
...
- 배포는 **OIDC 기반의 short-lived credential**로 바꾸고(`id-token: write`)[^2]
- `concurrency`로 중복 실행/배포 충돌을 시스템적으로 차단하며[^3]
- 캐시/러너 버전 호환성을 “운영”으로 끌어올리는 것[^4]
...
[^1]: <https://docs.github.com/en/actions/writing-workflows/workflow-syntax-for-github-actions?s=09>
...
[^3]: <https://docs.github.com/en/actions/using-jobs/using-concurrency>
[^4]: <https://github.com/actions/cache>
[^5]: <https://docs.github.com/en/actions/tutorials/use-github_token-in-workflows>
[^6]: <https://docs.github.com/en/rest/actions/permissions>
