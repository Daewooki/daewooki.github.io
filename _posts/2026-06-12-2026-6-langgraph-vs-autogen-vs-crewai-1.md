---
title: "멀티 에이전트 “진짜로” 굴리려면: LangGraph vs AutoGen vs CrewAI 심층 비교 & 구현 가이드"
description: "LLM 기반 기능이 “단일 Agent + Tool 몇 개” 수준을 넘어서면, 곧바로 오케스트레이션 문제가 터집니다."
...
2026년 6월 기준, 실무에서 가장 자주 비교되는 오픈소스 축은 **LangGraph, Microsoft AutoGen(AgentChat 계열), CrewAI**입니다. (최근 AutoGen+Semantic Kernel을 단일 프레임워크로 통합하려는 흐름도 커졌습니다.)[^1]

### 언제 쓰면 좋나
- **LangGraph**: “프로덕션 워크플로우 엔진”에 가까운 제어가 필요할 때(체크포인트/재개/승인/실패 처리/분기)[^2]  
- **AutoGen**: 여러 Agent의 **대화 패턴(그룹챗, 선택자, 종료조건 등)** 을 중심으로 설계하고 싶을 때(대화 기반 협업, 실험 속도)[^3]  
- **CrewAI**: Role/Task 중심으로 팀을 빠르게 구성하고, **sequential/hierarchical 프로세스**로 빨리 MVP를 뽑고 싶을 때[^4]  

### 언제 쓰면 안 되나
- 절차가 단순하고 상태가 짧은데도 “멀티 에이전트”로 과설계하면, 오히려 **토큰 비용 + 디버깅 난이도**만 증가합니다. 최근에는 **절차형 작업은 In-Context Prompting만으로도 충분한 경우가 늘었다**는 문제제기도 있습니다.[^5]  
...
- **Checkpointing/Persistence**: 각 실행 스텝의 state 스냅샷을 저장해서  
  ...
  - 장시간 작업의 **재개(resume)**
  를 가능하게 합니다.[^2]  
- “pending writes”처럼, 병렬 노드가 섞일 때도 **부분 성공을 기록**하는 쪽으로 설계되어 운영 안정성을 올립니다.[^2]  

즉 LangGraph는 “Agent 프레임워크”라기보다 **상태 기계(state machine)/워크플로우 엔진을 LLM에 맞춘 형태**에 가깝습니다. 대신 러닝커브가 있습니다(노드/엣지/리듀서/조건 라우팅 등).[^6]  

### 2) AutoGen: Multi-Agent Conversation + Runtime(상태 저장 훅)
AutoGen은 “그래프”보다는 **대화(conversation)** 가 1급 객체입니다.

- Agent들이 메시지를 주고받고, **termination condition**으로 종료를 판단합니다.[^3]  
- 커스텀 Agent를 만들면 `save_state()` / `load_state()`를 오버라이드해 상태를 저장/복원할 수 있습니다.[^7]  
...
- **Process**를 sequential 또는 hierarchical(매니저가 분배/검수/재할당)로 실행합니다.[^8]  
- 문서상으로는 guardrails, memory, knowledge, observability 등을 “내장” 방향으로 강조합니다.[^4]  
...
def approval_gate_node(state: AgentState) -> AgentState:
    ...
    # LangGraph는 interrupt 시점에 state를 persistence로 저장해두고 재개 가능.[^9]
    decision = interrupt({
...
def build_app(sqlite_path: str = "agent_graph.sqlite"):
    ...
    # LangGraph는 checkpointing/persistence로 멀티턴/복구/승인 워크플로우를 지원.[^10]
...
- risk가 `high`이면: `interrupt()`에서 멈추며, **그 시점 state가 SQLite에 저장**됩니다(프로세스가 죽어도 복구 가능).[^9]  
...
1) **State를 작게 유지**
- LangGraph 체크포인트는 강력하지만, state가 커질수록 **저장 비용/직렬화 오버헤드**가 늘어납니다. “원문 전체” 대신 요약/구조화 결과만 state에 넣고, 원문은 외부 스토리지(S3/DB)에 두고 key만 들고 가세요. (LangGraph 문서에서도 state를 작게 유지하라고 강조)[^11]  

2) **승인/중단 지점을 ‘명시적으로’ 설계**
- human-in-the-loop는 “가끔 사람에게 물어보기”가 아니라, **중단 가능한 실행 모델 + 재개 전략**이 있어야 운영됩니다. LangGraph는 interrupt 시 persistence로 상태 저장 후 대기하는 패턴을 공식 가이드로 제공합니다.[^9]  
...
- **Long-term memory를 무비판적으로 축적**
  → 2026년 연구에서도 장기 메모리는 cross-domain leakage, memory-induced sycophancy 같은 리스크가 지적됩니다. 개인화가 필요하면 “저장 정책/만료/스코프 분리”를 설계하세요.[^12]  
...
또한, “외부 오케스트레이션이 과연 항상 필요한가?”에 대한 반론도 있으니(모델 성능 향상으로 절차형 작업은 프롬프트로 대체 가능) 작업 성격을 냉정히 분류하세요.[^5]  
...
- “장시간/실패/재개/승인/감사로그”가 핵심이면 → **LangGraph** (checkpoint+interrupt가 게임 체인저)[^2]  
- “역할 기반 팀을 빠르게 꾸려 결과를 뽑고, 프로세스는 sequential/hierarchical이면 충분” → **CrewAI**[^4]  
- “멀티 에이전트 대화 패턴을 중심으로 다양한 상호작용을 실험/확장” → **AutoGen(AgentChat)**, 필요 시 상태 저장 훅을 적극 활용[^3]  

### 다음 학습 추천(실무 순서)
1) LangGraph의 **persistence/checkpoint + interrupt**를 먼저 잡고(운영성)[^2]  
...
[^1]: <https://www.reddit.com/r/AutoGenAI/comments/1o0p73u/autogen_semantic_kernel_microsoft_agent_framework/>
[^2]: <https://langchain-5e9cc07a.mintlify.app/oss/python/langgraph/persistence>
[^3]: <https://microsoft.github.io/autogen/stable/reference/python/autogen_agentchat.base.html>
[^4]: <https://docs.crewai.com/>
[^5]: <https://arxiv.org/abs/2604.27891>
[^6]: <https://thinking.inc/en/tool-comparisons/langgraph-vs-crewai-vs-autogen/>
[^7]: <https://microsoft.github.io/autogen/dev/user-guide/agentchat-user-guide/tutorial/state.html>
[^8]: <https://learn.engineering.vips.edu/agent-protocols/crewai-hierarchical-process>
[^9]: <https://langchain-5e9cc07a.mintlify.app/oss/python/langgraph/human-in-the-loop>
[^10]: <https://langchain-ai.github.io/langgraph/reference/checkpoints/?h=langgraph+checkpoint+sqlite+import+saver>
[^11]: <https://langgraphjs.guide/persistence/>
[^12]: <https://arxiv.org/abs/2602.01146>
