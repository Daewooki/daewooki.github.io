---
title: "429가 “가끔”이 아니라 “구조적으로” 터지는 시대: LLM API Rate Limit 대응 Retry/Backoff 패턴 심층 분석"
description: "LLM API를 프로덕션에서 돌리다 보면 429(Too Many Requests / RESOURCEEXHAUSTED)은 “내가 분당 한도를 넘겼다” 수준의 단순 문제가 아닙니다."
...
## 들어가며
LLM API를 프로덕션에서 돌리다 보면 429(Too Many Requests / RESOURCE_EXHAUSTED)은 “내가 분당 한도를 넘겼다” 수준의 단순 문제가 아닙니다. **짧은 버스트(burst)로 인한 quantization**, **토큰 기반 제한(TPM/ITPM/OTPM)과 요청 기반 제한(RPM)의 동시 작동**, **조직/워크스페이스 단위 bucket**, **스펜드(spend) 기반 제한**까지 겹치면서 “대시보드상 여유가 있어도 429가 나는” 상황이 흔합니다. OpenAI는 한도가 더 짧은 구간으로 쪼개져 집행될 수 있음을 명시하고, 해결책으로 exponential backoff를 권장합니다.[^1] Anthropic은 token bucket 기반임을 문서에 적고, 429에 `retry-after`가 포함될 수 있음을 명시합니다.[^2] Google Gemini도 429/503에 대해 exponential backoff(+jitter)를 권장하고, SDK에 기본 retry가 들어있다고 안내합니다.[^3]
...
2026년 기준 주요 LLM API는 보통 **요청 수(RPM)** 와 **토큰(입력/출력 TPM)** 을 동시에 제한합니다. Anthropic은 이를 명시적으로 RPM/ITPM/OTPM으로 설명하고, 내부적으로 token bucket 알고리즘으로 집행한다고 밝힙니다.[^2] OpenAI도 분당 한도라도 더 짧은 시간 단위로 “quantized enforcement” 될 수 있어 순간 버스트에 취약함을 적고 있습니다.[^1]
...
가장 좋은 클라이언트는 429를 받았을 때 **(1) 서버가 주는 `retry-after`를 최우선으로 존중**하고, 없으면 **(2) 지수 backoff + jitter**로 “군집 재시도(thundering herd)”를 피합니다. Anthropic 문서에는 429에 `retry-after`가 포함될 수 있고 “그 전에 재시도하면 실패한다”고 명확히 적혀 있습니다.[^2]  
Gemini도 429/503에 대해 exponential backoff(+jitter)를 권장합니다.[^3]
...
OpenAI가 말하는 “짧은 버스트가 한도 에러를 만든다”는 문제는, retry만으로는 근본 해결이 안 됩니다(계속 버스트를 만들면 계속 맞습니다).[^1]
...
- 가정: LLM 공급자는 429에 `Retry-After`를 줄 수도/안 줄 수도 있음(Anthropic은 준다고 명시, Gemini/OpenAI는 상황별)[^2]
...
async def worker(name: str, queue: asyncio.Queue, pacer: GlobalPacer, client: httpx.AsyncClient, url: str):
    ...
    while True:
        job = await queue.get()
        try:
            # 1) 사전 페이싱: 버스트 억제 (OpenAI의 quantization 이슈를 실무에서 가장 많이 줄여줌)[^1]
            await pacer.acquire()

            payload = {
                "input": job["prompt"],
                # 현실 포인트: max tokens를 과하게 잡으면 "예상 사용량"이 커져 rate limit에 불리할 수 있음(OpenAI 권고)[^1]
                "max_output_tokens": job.get("max_output_tokens", 512),
                "temperature": job.get("temperature", 0.2),
                "metadata": {"job_id": job["id"], "worker": name},
            }

            # 2) 사후 복구: 429/503면 retry-after 우선 + exp backoff + jitter (Gemini 권고)[^3]
            @retry_call
            async def _do():
...
- 429가 나면 **Retry-After를 가장 먼저 신뢰**하고(Anthropic이 명시)[^2]
- 없으면 **지수 backoff + jitter**로 분산시키는 점입니다(Gemini 권고).[^3]

---

## ⚡ 실전 팁 & 함정
### Best Practice 1) “Retry-After 우선, 없으면 exp backoff”를 표준화하라
- Anthropic은 429에 `retry-after`가 오며 “그 전에 재시도하면 실패한다”고 적습니다.[^2]  
...
OpenAI는 분당 한도라도 짧은 구간으로 쪼개 집행되어 버스트에 의해 429가 날 수 있다고 명시합니다.[^1]  
...
OpenAI는 max tokens 값이 “예상 사용량”에 영향을 주고, 과도하게 잡으면 rate limit에 불리해질 수 있다고 조언합니다.[^1]  
...
- 429/408/5xx 위주로 제한하고, 공급자 문서가 명시한 코드만 허용하는 편이 안전합니다(Gemini 가이드도 “특정 에러만 재시도” 권장).[^3]
...
- **비용↑/↓ 둘 다 가능**
  - 재시도 자체는 호출 수를 늘려 비용↑
  - 반대로 “불필요한 max tokens 축소”로 토큰 비용↓ 가능(OpenAI 조언)[^1]
...
1) **버스트를 없애는 전역 페이싱**(quantization/짧은 구간 집행 대응)[^1]  
2) **Retry-After가 있으면 최우선 준수**(특히 Anthropic은 명시)[^2]  
3) 없으면 **exponential backoff + jitter**(Gemini 포함 다수 가이드가 권장)[^3]  
4) **max tokens를 현실적으로 줄여** 토큰 기반 제한과 비용을 같이 관리[^1]

**도입 판단 기준**
- 동시성(워커 수)과 트래픽 버스트가 존재한다 → “retry만” 말고 “pacer+retry”로 가야 합니다.
- 429가 하루 종일 지속된다/즉시 429가 난다 → 단순 backoff로는 해결이 안 될 수 있으니, 키/조직/티어/스펜드 제한/엔드포인트 특이 제한을 함께 점검해야 합니다(Gemini는 spend 기반 429도 문서화).[^4]

**다음 학습 추천**
- 공급자별 rate limit 헤더/Limit API를 읽고(Anthropic은 Rate Limits API와 헤더를 문서화)[^2], 내 서비스에 맞는 “전역 페이싱(분산 레이트리미터)”를 설계해보세요.
- 장기적으로는 “클라이언트 측 제어 알고리즘” 연구도 흥미롭습니다(백오프 대비 429를 크게 줄이는 접근을 다루는 최근 연구가 나와 있습니다).[^5]

[^1]: <https://help.openai.com/en/articles/6891753-api-rate-limit-advice>
[^2]: <https://platform.claude.com/docs/en/api/rate-limits>
[^3]: <https://ai.google.dev/gemini-api/docs/troubleshooting>
[^4]: <https://ai.google.dev/gemini-api/docs/rate-limits?authuser=2>
