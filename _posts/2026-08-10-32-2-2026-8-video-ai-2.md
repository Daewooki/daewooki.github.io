---
title: "프레임 예산 32장으로 2시간짜리 영상을 “이해/생성”하기: Video AI 파이프라인 설계 가이드"
description: "2026년 8월 시점의 video AI는 “모델이 곧 비디오를 통째로 본다”기보다, 긴 비디오를 제한된 token/window에 맞게 ‘증거(evidence)’로 압축해 넣는 파이프라인 싸움에 가깝습니다."
...
- **Query-aware keyframe selection**: “지금 질문에 필요한 장면”을 우선 선택  
  - training-free로도 동작하는 모듈(plug-and-play)을 지향하는 흐름(예: FOCUS 같은 키프레임 선택 모듈)[^4]
  - 자막/오디오/텍스트 단서를 섞어 키프레임 선택을 보조하는 흐름도 존재(VSI 등)[^5]
...
> 모델 호출은 환경마다 다르니, 여기서는 `analyze_frame()`을 “Gemini Video understanding 또는 내부 VLM 호출”로 교체 가능한 형태로 작성했습니다. Gemini는 비디오 이해 API 가이드를 공식 제공 중입니다.[^2]
...
정답 자체보다 “정답에 필요한 장면을 모델 컨텍스트에 넣었는가”가 더 1차적인 실패 원인입니다. 키프레임 선택을 연구하는 흐름(FOCUS 등)도 바로 이 지점을 겨냥합니다.[^4]

3) **오디오/자막을 프레임 선택에 섞어라**
Gemini의 비디오 이해는 오디오+비주얼을 같이 처리하는 문서 흐름을 명시합니다.[^2]  
실무에서는 ASR 타임라인에서 키워드 구간을 찾고, 그 주변 N초만 dense frame으로 뽑는 게 비용 대비 효과가 큽니다.

### 흔한 함정/안티패턴
- **“1fps로 다 넣자”**: File API가 1fps로 저장되는 특성이 있더라도[^2], 2시간이면 7200프레임입니다. 결국 또 줄여야 하며, 줄이는 로직이 없으면 성능/비용이 폭발합니다.
...
## 🚀 마무리
핵심은 “비디오를 이해하는 모델”보다 **비디오를 ‘증거 묶음’으로 변환하는 프레임 분석 파이프라인**입니다. 2026년 8월의 최신 흐름은 (1) query-aware keyframe/segment selection, (2) 멀티턴/에이전트식 증거 수집, (3) 오디오·자막·OCR을 결합한 멀티모달 근거화로 수렴하고 있습니다.[^1]
...
**다음 학습 추천**
- LVU용 멀티턴 증거 수집: VideoMind, Video-MTR[^1]
- 키프레임 선택 모듈(plug-and-play): FOCUS 계열[^4]
- 상용 API의 제약(프레임레이트/오디오 처리/파일 파이프라인): Gemini video understanding 문서[^2]

[^1]: <https://aclanthology.org/2026.eacl-industry.30/>
[^2]: <https://ai.google.dev/gemini-api/docs/video-understanding?hl=en>
[^3]: <https://openreview.net/forum?id=UhPwL6LYOc>
[^4]: <https://arxiv.org/abs/2510.27280>
[^5]: <https://arxiv.org/abs/2508.06869>
[^6]: <https://docs.oracle.com/en/solutions/agentic-video-analysis/index.html>
